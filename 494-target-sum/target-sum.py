class Solution:
    def findTargetSumWays(self, nums: List[int], target: int) -> int:
        ...
        def helper(ind, target, dp):
            ...
            notPick = helper(ind - 1, target, dp)
            pick = 0
            if( nums[ind] <= target):
                pick = helper(ind - 1, target - nums[ind], dp)
        ...
        dp = [[-1 for _ in range(neededTarget+1)] for _ in range(n)]

        return helper(n-1, neededTarget, dp)